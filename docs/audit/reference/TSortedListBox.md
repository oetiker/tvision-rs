...
| `handleEvent` (method) | 535 | PORTED | SUSPECT | `tv::list_viewer::sorted_handle_event(this, ev, ctx)` free function | 3 | C++ sequence: save `oldValue`, call base `TListBox::handleEvent`, reset `searchPos=-1` if focused moved OR `cmReleasedFocus`, then gate on `evKeyDown && charCode != 0`, run kbBack/dot/char branches. Rust mirrors this exactly. **SUSPECT**: C++ `setCursor(cursor.x + (searchPos - oldPos), cursor.y)` on the "same item, cursor advance" path (stddlg.cpp line 172) is a mutable cursor-state write inside `handleEvent`. Rust derives the cursor ABSOLUTELY via `sorted_cursor` (a `cursor_request` return) rather than accumulating it — this is a documented deviation (see `sorted_cursor` doc). The Rust approach is correct and documented. **However**, the Rust `sorted_handle_event` does NOT distinguish the "same item / cursor advance only" path (C++ stddlg.cpp:170-173) from the "new item" path — it always runs `focus_item` when the value changes and simply lets `sorted_cursor` recompute. In practice `sorted_cursor` returns the correct position each frame, so behavior is equivalent. Classify as OK on re-examination. No actual suspect found — reclassify to PORTED / OK. |
...
- SUSPECT: 1   |   doc<3 (public): 4   |   → concept: 0
