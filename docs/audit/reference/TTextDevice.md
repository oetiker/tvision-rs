...
| `CTextDevice` palette (inherited) | 557 | EQUIVALENT | OK | `Role::ScrollerNormal` + `Role::ScrollerSelected` via `Theme` (inherited from `Scroller`) | 2 | `TTextDevice` has no own palette; it inherits `TScroller`'s `CScroller` (2-entry `cpScroller`). Rust `Terminal` similarly inherits the scroller role mapping. Known mapping: class Palette → `tv::Theme` (D7). **Deferred to theme pass** — score 2 left as-is (theme chain lives in `src/theme.rs`, not `terminal.rs`). |

## Summary

- PORTED: 1   EQUIVALENT: 3   NOT-PORTED: 3   MISSING: 0   UNSURE: 0
- SUSPECT: 0   |   doc<3 (public): 2 (both deferred — `GetPalette` is a `→ concept` row; `CTextDevice` palette deferred to theme pass)   |   → concept: 1
- Notable finding: All three NOT-PORTED entries (`overflow`, `xsputn`, `otstream`) are C++ `streambuf`/`ostream` plumbing with no equivalent in Rust's I/O model; their absence is correct and follows documented deviations D11/D12. No gaps. The `GetPalette` chain (D7 mapping to `Role::ScrollerNormal`) could use an explicit note in the `TextDevice` trait doc pointing to the palette chain concept page. The 2 below-bar rows are structural deferrals: the `GetPalette` row is a `→ concept` entry (leave as-is) and the palette row maps to `src/theme.rs` (theme-pass deferred).
