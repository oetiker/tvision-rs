...
|---|---|---|---|---|---|---|
| `background` (field, `TBackground*`) | 412 | EQUIVALENT | OK | `Desktop.background: Option<ViewId>` | 2 | C++ stores a raw pointer; Rust uses a `ViewId` handle (deviation D3). The field is private but exposed via `Desktop::background() -> Option<ViewId>`. Doc says what it is but not the "sent-behind-background-on-cmPrev" usage. |
