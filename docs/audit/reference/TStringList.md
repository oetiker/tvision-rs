...
| `Get` (method) | 549 | EQUIVALENT | OK | `tv::text::StringList::get(key: u16) -> Option<&str>` | 2 | C++: `Get(Key: Word): String` returns `""` for missing keys. Rust returns `Option<&str>`; callers use `.unwrap_or("")` for the same default. Semantic contract preserved; shape is idiomatic Rust. Doc explains *what*, not *how/when to use* in context. |
...
- Notable finding: The sole public capability (`Get`) is faithfully preserved as `tv::text::StringList::get`; all dropped entries are stream/persistence machinery (resource/streamable subsystem dropped project-wide, deviation D12). The `TStringList`/`TStrListMaker` maker-vs-reader split collapses into one type because the write/read streaming asymmetry no longer exists.
