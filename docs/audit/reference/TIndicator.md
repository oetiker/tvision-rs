...
| `GetPalette` (method) | 459 | EQUIVALENT | OK | `tv::Role::IndicatorNormal` + `tv::Role::IndicatorDragging` via `tv::Theme` | 2 | Deferred to theme pass — `Role::IndicatorNormal` and `Role::IndicatorDragging` live in `src/theme.rs` (constraint: do not edit theme.rs in this pass). |
...
| `CIndicator` palette (2 entries) | 459 | EQUIVALENT | OK | `tv::Role::IndicatorNormal`, `tv::Role::IndicatorDragging` | 2 | Deferred to theme pass — both `Role` variants live in `src/theme.rs` (constraint: do not edit theme.rs in this pass). |
...
- Raised to 3: `location`, `modified`, `new`, `set_value` (4 symbols).
- Remaining doc<3: `GetPalette`/`CIndicator` rows → deferred to theme pass (`Role::IndicatorNormal`, `Role::IndicatorDragging` in `src/theme.rs`); `SetState` → no Indicator override exists, adding a doc comment would require a code change (out of scope).
