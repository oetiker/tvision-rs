...
| B8 ✅ | small singletons | — | **COMPLETE (`dae38c1`):** `max_len` clamp on `InputLine::set_value` (UTF-8-safe truncation, mirrors paste_text); `valid-select` (`ctx.request_focus(id)` before return false, faithful to C++ `select()`). Timer-payload pre-resolved (`Event::Timer(id)` already in pump idle). `init/doneHistory` + help-ctx propagation remain standing deferrals (need history subsystem port + OneOf status line respectively). |
...
| C7 ✅ | help-ctx refresh / `OneOf` status line | **COMPLETE (`8c9bf85`+`20871fd`):** `View::get_help_ctx()` trait method (default: `self.state().get_help_ctx()`); forwarder in `tvision-macros/src/specs.rs`; `delegate_view` spy updated (27 methods). `Program::pump_once` idle arm wires `TStatusLine::update()`: reads `captures.top_modal_view()` as rstv's `TheTopView` equivalent, calls `v.get_help_ctx()` on the modal, forwards to `sl.set_help_ctx(top_ctx)`. `set_help_ctx` now idempotent (early-return guard mirrors C++ `if(helpCtx!=h)`). OneOf defs switch automatically when a modal with a matching `helpCtx` is the top view. |
