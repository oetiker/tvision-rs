...
next**. As of this writing the **faithful-port ladder is complete**: every row
**1–92 is ✅** except **81–87** (the `TColorDialog` cluster), which is
**~~DROPPED~~** as a D7 consequence and superseded by the truecolor color-picker
extension (see row 81). The Phase C feature backlog (`C1`–`C9` — find/replace,
context menu, internal clipboard, dialog gather/scatter, cmQuit veto, dosShell,
help-ctx refresh, theme editor, bracketed paste) also landed; the
forward-looking state lives in [`docs/HANDOVER.md`](file:///home/oetiker/checkouts/rstv/docs/HANDOVER.md).
...
| 66 ✅ | `TEditor` | `TView` | `teditor1.cpp`, `teditor2.cpp`, `edits.cpp` | `widgets::editor` | FOUNDATION | **✅ complete (core + all breadcrumbs cleared).** Gap buffer + nav + edit + undo + selection + draw + keyboard handleEvent + search + D3 brokers (new `SyncEditorDelta`/`IndicatorSetValue` + clipboard `SetClipboard`/`EditorPaste`; `Role::ScrollerSelected` added) + system-clipboard cut/copy/paste. **The original breadcrumbs all landed later:** mouse drag-select/edge-scroll/wheel (`694963d`, B2, via the mouse-track seam); find/replace dialogs + `do_search_replace` (`b388492`, C1, via the **interactive modal-from-view seam** — `docs/design/async-modal-from-view.md`); right-click context menu (`2ee829c`, C2); internal-clipboard-editor branch →69 (`9e88983`, C3). **Only genuine drop:** `TStreamable` (D12). |
...
| 80 ✅ | `TChDirDialog` | `TDialog` | `tchdrdlg.cpp`, `schdrdlg.cpp`, `nmchdrdl.cpp` | `dialog` (filedlg) | MECHANICAL | **✅** `ChDirDialog` = D2 embed-delegate over `Dialog` (the `FileDialog` precedent); assembles `InputLine`+`THistory`+`ScrollBar`+row-75 `DirListBox`+OK/Chdir/Revert/[Help] buttons. `valid(cmOK)` does the **real** `chdir` (`std::env::set_current_dir`; faithfully does NOT chain to `TDialog::valid`). Resolved the two row-75 breadcrumbs: `select_item`→`ctx.post(cmChangeDir)` (dialog reads `focused_entry()` directly), `set_state`→new **`Deferred::MakeButtonDefault`** broker (`chDirButton->makeDefault`). D14 native `/` paths; D10 `dataSize=0`→value/set_value trait-default. D12 streaming dropped. |
| 81 ~~DROPPED~~ | `TColorItem`/`TColorGroup`/`TColorIndex` | — | `colorsel.h` (inline), `sclrsel.cpp` | `dialog` (colordlg) | MECHANICAL | **DROPPED (D7 consequence):** the entire `TColorDialog` cluster (81–87) edits `app->palette->data[index]` — the flat runtime-mutable BIOS palette that rstv deletes under D7 (palette → `Theme`; `Role` is a closed enum; `WindowPalette` is only a Blue/Cyan/Gray tag). A faithful port would edit a `TPalette` nothing reads — dead code by construction. Rows 81–82 were temporarily landed then **reverted** (`9aa8e12`). The cluster is superseded by the **truecolor color-picker extension** (`src/dialog/colorpick/`, `Program::color_dialog`) — see spec [`docs/superpowers/specs/2026-06-09-color-picker-design.md`](docs/superpowers/specs/2026-06-09-color-picker-design.md) and plan [`docs/superpowers/plans/2026-06-09-color-picker.md`](docs/superpowers/plans/2026-06-09-color-picker.md). Faithful port resumes at **row 88**. |
| 82 ~~DROPPED~~ | `TColorSelector` | `TView` | `colorsel.cpp`, `sclrsel.cpp` | `dialog` (colordlg) | MECHANICAL | **DROPPED** — see row 81. |
| 83 ~~DROPPED~~ | `TColorDisplay` | `TView` | `colorsel.cpp`, `sclrsel.cpp` | `dialog` (colordlg) | MECHANICAL | **DROPPED** — see row 81. |
| 84 ~~DROPPED~~ | `TMonoSelector` | `TCluster` | `colorsel.cpp`, `sclrsel.cpp` | `dialog` (colordlg) | MECHANICAL | **DROPPED** — see row 81. |
