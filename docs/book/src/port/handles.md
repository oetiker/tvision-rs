...
- **Up and sideways are handles.** Every link that used to be a `TView*`
  ...
  broadcast subject — becomes a [`ViewId`](../api/tvision/view/struct.ViewId.html):
...
into the view's own [`ViewState`](../api/tvision/view/struct.ViewState.html). A
view therefore knows its own handle; ask for it with
[`id()`](../api/tvision/view/struct.ViewState.html#method.id), which returns
...
borrowed [`Context`](../api/tvision/view/struct.Context.html): the owner's size,
...
[`find_mut`](../api/tvision/view/trait.View.html#method.find_mut) tree-walk: a
...
[`Event::Broadcast`](../api/tvision/event/enum.Event.html) carrying a `command`
