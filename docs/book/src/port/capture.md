...
**one mechanism** in rstv: the **capture stack**.
...
[`src/capture.rs`](../api/tvision/capture/index.html) and is driven by the one
event loop in [`Program`](../api/tvision/app/struct.Program.html).

## The stack

The event loop owns a single
[`CaptureStack`](../api/tvision/capture/struct.CaptureStack.html) — a LIFO stack
...
[`CaptureHandler`](../api/tvision/capture/trait.CaptureHandler.html) and answers
with a [`CaptureFlow`](../api/tvision/capture/enum.CaptureFlow.html):
...
A handler holds a [`ViewId`](../api/tvision/view/struct.ViewId.html) for the view
...
- **Window drag and resize.** Grabbing a window's title bar or a resize corner in
  the [`Frame`](../api/tvision/frame/struct.Frame.html) pushes a drag handler that
...
- **Press-and-hold across the widgets.** When you press and hold on a control,
  ...
  Roughly ten widgets rely on this — [`Button`](../api/tvision/widgets/struct.Button.html),
  [`Cluster`](../api/tvision/widgets/struct.Cluster.html),
  [`Editor`](../api/tvision/widgets/struct.Editor.html),
  [`InputLine`](../api/tvision/widgets/struct.InputLine.html),
  [`ScrollBar`](../api/tvision/widgets/struct.ScrollBar.html),
  `ListViewer`, `Outline`, the status line, the menus, and the window
  [`Frame`](../api/tvision/frame/struct.Frame.html) — each for its own gesture
