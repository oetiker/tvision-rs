...
through its [`Context`](../api/tvision/view/struct.Context.html), and the event
...
- **any other view** addressed by [`ViewId`](../api/tvision/view/struct.ViewId.html)
...
[`Context`](../api/tvision/view/struct.Context.html) carries a single
...
brokers, …) each push one [`Deferred`](../api/tvision/view/enum.Deferred.html)
variant onto it. The view never sees the apply step.

After the dispatch returns, [`pump_once`](../api/tvision/app/struct.Program.html)
...
1. **Add a variant** to [`Deferred`](../api/tvision/view/enum.Deferred.html),
