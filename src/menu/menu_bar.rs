...
//! [`menu_view::handle_event`] (the C++ `TMenuBar` does not override
//! `TMenuView::handleEvent`): that handles the command-graying broadcast, posts an
//! accelerator's command, and opens the menu session on a bar alt-shortcut.
//!
...
//! first `cmCommandSetChanged`, which does not fire at startup; so
...
//! Ports `TMenuBar` (`tmenubar.cpp`/`menus.h`). C++ inheritance from `TMenuView`
//! becomes the [`MenuView`] trait (deviation D2); the disabled-command set is held
//! as a denylist (deviation D1) and `TStreamable` persistence is dropped
//! (deviation D12).
...
impl View for MenuBar {
    ...
    /// view-local [`DrawCtx`] writes (no separate draw buffer).
    ...
    /// `TMenuView::handleEvent`, so this delegates to the shared passive
    /// [`menu_view::handle_event`] (command-graying broadcast, accelerator post,
    /// and bar alt-shortcut activation).
    fn handle_event(&mut self, ev: &mut Event, ctx: &mut Context) {
        menu_view::handle_event(&self.mv, ev, ctx);
    }

    /// The command-graying broker hook (`TMenuView::updateMenu`). The pump
    /// calls this at apply time (via [`Deferred::UpdateMenu`]) with the live
    /// **disabled-command set** in hand, and the ctor calls it
