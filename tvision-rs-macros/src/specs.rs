...
fn view(f: &Ident, k: &TokenStream) -> Vec<(&'static str, TokenStream)> {
    vec![
        ...
        ("has_focusable_leaf",
         quote! { fn has_focusable_leaf(&self) -> bool { self.#f.has_focusable_leaf() } }),
        ("focus_to_edge",
         quote! { fn focus_to_edge(&mut self, backward: bool, ctx: &mut #k::Context) -> bool { self.#f.focus_to_edge(backward, ctx) } }),
        ("reset_current",
